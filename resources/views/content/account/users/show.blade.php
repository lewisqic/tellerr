...
@section('content')
    ...
    {!! Breadcrumbs::render('account/users/show', $user) !!}

    <div class="float-right">
        <a href="{{ url('account/users/' . $user->id . '/edit?_ajax=false&_redir=' . urlencode(url('account/users/' . $user->id))) }}" class="btn btn-primary open-sidebar"><i class="fa fa-edit"></i> Edit</a>
        <form action="{{ url('account/users/' . $user->id) }}" method="post" class="validate d-inline ml-2" id="delete_form">
            ...
            <div class="tab-content mt-4">
                <div class="tab-pane active" id="show_details" role="tabpanel">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10 form-control-static">
                            {!! $user->last_login ? $user->last_login->toDayDateTimeString() : '<em class="text-muted">never</em>' !!}
                        </div>
                    </div>
                        ...
                        <div class="col-sm-10 form-control-static">
                            @if ( $user_roles )
                                @foreach ( $user_roles as $role )
                                    <div><a href="{{ url('account/roles/' . $role['id']) }}">{{ $role['name'] }}</a></div>
                                @endforeach
                            @else
